...
# # Validation schemas
# CLIMATE_TIMER_SCHEMA: vol.Schema = vol.Schema({
#     vol.Required(ATTR_TEMPERATURE): vol.Coerce(float),
#     vol.Exclusive(
#         ATTR_TIME_PERIOD, DOMAIN
#     ): vol.All(cv.time_period, cv.positive_timedelta, lambda td: td.total_seconds()),
#     vol.Exclusive(
#         ATTR_REQUESTED_OVERLAY, DOMAIN
#     ): vol.In(CONST_OVERLAY_TADO_OPTIONS),
# })

# CLIMATE_TEMP_OFFSET_SCHEMA: vol.Schema = vol.Schema({
#     vol.Required(ATTR_OFFSET, default=0): vol.Coerce(float),
# })
...
# def _generate_entities(tado: TadoConnector) -> list[ClimateEntity]:
#     """Create all climate entities."""
...
#     # Handle heat temperatures
#     if CONST_MODE_HEAT in capabilities:
#         heat_temperatures = capabilities[CONST_MODE_HEAT].get("temperatures")
#     elif "temperatures" in capabilities:
#         heat_temperatures = capabilities["temperatures"]

#     # If no temperatures are available, skip entity creation
#     if not heat_temperatures and not cool_temperatures and not tado.is_x:
#         _LOGGER.debug("Not adding zone %s since it has no temperatures", name)
#         return None
...
#         heat_min_temp = float(heat_temperatures.get("celsius", {}).get("min", TADO_X_DEFAULT_MIN_TEMP))
#         heat_max_temp = float(heat_temperatures.get("celsius", {}).get("max", TADO_X_DEFAULT_MAX_TEMP))
#         heat_step = heat_temperatures.get("celsius", {}).get("step", PRECISION_TENTHS)

#     if cool_temperatures:
#         cool_min_temp = float(cool_temperatures.get("celsius", {}).get("min"))
#         cool_max_temp = float(cool_temperatures.get("celsius", {}).get("max"))
#         cool_step = cool_temperatures.get("celsius", {}).get("step", PRECISION_TENTHS)
...
#         supported_swing_modes: list[str] | None = None,
#     ) -> None:
#         """Initialize Tado climate entity."""
#         self._tado = tado
#         super().__init__(zone_name, tado.home_id, zone_id)
...
#         # Load capabilities and data
#         self._current_capabilities = tado.get_capabilities(zone_id)
#         self._tado_zone_data: PyTado.TadoZone = {}
#         self._tado_geofence_data: dict[str, str] | None = None
#         self._tado_zone_temp_offset: dict[str, Any] = {}
...
#             async_dispatcher_connect(
#                 self.hass,
#                 SIGNAL_TADO_UPDATE_RECEIVED.format(self._tado.home_id, "zone", self.zone_id),
#                 self._async_update_zone_callback,
#             )
#         )
...
#         _LOGGER.debug(
#             "Setting temperature offset for device %s to %.1f",
#             self._device_id,
#             offset,
#         )
#         self._tado.set_temperature_offset(self._device_id, offset)
...
#             return
#         new_hvac_mode = (
#             HVACMode.COOL if self._is_ac_device() else HVACMode.HEAT
#         )
#         self._control_hvac(target_temp=temperature, hvac_mode=new_hvac_mode)
...
#     @property
#     def swing_mode(self) -> str | None:
#         # Determine current swing mode based on vertical, horizontal, and main swing modes
#         modes = (
#             self._current_tado_swing_mode,
#             self._current_tado_vertical_swing,
#             self._current_tado_horizontal_swing,
#         )
#         if modes == (TADO_SWING_OFF, TADO_SWING_OFF, TADO_SWING_OFF):
#             return TADO_TO_HA_SWING_MODE_MAP[TADO_SWING_OFF]
#         if modes == (TADO_SWING_ON, TADO_SWING_OFF, TADO_SWING_OFF):
#             return TADO_TO_HA_SWING_MODE_MAP[TADO_SWING_ON]
#         if modes == (TADO_SWING_OFF, TADO_SWING_ON, TADO_SWING_OFF):
#             return SWING_VERTICAL
#         if modes == (TADO_SWING_OFF, TADO_SWING_OFF, TADO_SWING_ON):
#             return SWING_HORIZONTAL
#         if modes == (TADO_SWING_OFF, TADO_SWING_ON, TADO_SWING_ON):
#             return SWING_BOTH
#         return TADO_TO_HA_SWING_MODE_MAP[TADO_SWING_OFF]
...
#         attrs: dict[str, Any] = self._tado_zone_temp_offset.copy()
#         attrs[HA_TERMINATION_TYPE] = self._tado_zone_data.default_overlay_termination_type
#         attrs[HA_TERMINATION_DURATION] = self._tado_zone_data.default_overlay_termination_duration
#         return attrs
...
#     @callback
#     def _async_update_zone_data(self) -> None:
#         """Update zone data from Tado."""
#         self._tado_zone_data = self._tado.data["zone"][self.zone_id]
#         # Update offset based on device type
#         if self._tado.is_x:
#             self._tado_zone_temp_offset["offset_celsius"] = self._tado.data["device"][self._device_id][TEMP_OFFSET]
#         else:
#             for offset_key, attr in TADO_TO_HA_OFFSET_MAP.items():
#                 if (
#                     self._device_id in self._tado.data["device"]
...
#     def _normalize_target_temp_for_hvac_mode(self) -> None:
#         """Adjust target temperature within min/max bounds."""
...
#         # Initialize target temp if None
#         if self._target_temp is None:
#             self._target_temp = self._tado_zone_data.current_temp
#         elif self._current_tado_hvac_mode == CONST_MODE_COOL:
#             self._target_temp = adjust_temp(self._cool_min_temp, self._cool_max_temp)
...
#         # Handle off and smart schedule modes
#         if self._current_tado_hvac_mode == CONST_MODE_OFF:
...
#         # Determine overlay mode and duration
#         overlay_mode = decide_overlay_mode(
#             tado=self._tado,
#             duration=duration,
#             overlay_mode=overlay_mode,
...
#         _LOGGER.debug(
#             "Applying HVAC mode %s to zone %s (%d) with temp %.1f°C for %s seconds via overlay %s",
#             self._current_tado_hvac_mode,
#             self.zone_name,
#             self.zone_id,
#             self._target_temp,
...
#         # Prepare fan and swing parameters
#         fan_speed = None
#         fan_level = None
#         if self.supported_features & ClimateEntityFeature.FAN_MODE:
#             if self._is_current_setting_supported_by_hvac_mode(
#                 TADO_FANSPEED_SETTING, self._current_tado_fan_speed
#             ):
#                 fan_speed = self._current_tado_fan_speed
#             if self._is_current_setting_supported_by_hvac_mode(
...
#     def _is_valid_setting_for_hvac_mode(self, setting: str) -> bool:
#         """Check if a setting is supported for current HVAC mode."""
#         return (
#             self._current_tado_capabilities.get(self._current_tado_hvac_mode, {}).get(setting)
#             is not None
#         )

#     def _is_current_setting_supported_by_hvac_mode(self, setting: str, current_state: Optional[str]) -> bool:
#         """Check if current setting is supported in current HVAC mode."""
#         if self._is_valid_setting_for_hvac_mode(setting):
#             return current_state in self._current_tado_capabilities[self._current_tado_hvac_mode].get(setting, [])
#         return False